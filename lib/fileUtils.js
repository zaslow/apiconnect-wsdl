    ...
    modules[protocol].get(options, function(response) {
      ...
      response.on('end', function() {
        if (response.statusCode >= 200 && response.statusCode < 300) {
          const content = Buffer.concat(strings);
          let encoding;
          const left = content.toString('utf8', 0, 4);
          if (left == 'PK\u0003\u0004') {
            // treat as binary
            encoding = null;
          } else {
    ...
    try {
      fs.readFile(fullPath, function(err, rawData) {
        if (err) {
          ...
          let encoding = 'utf8'; // start with a common default encoding
          const left = rawData.toString('utf8', 0, 4);
          if (left == 'PK\u0003\u0004') {
            // treat as binary
            encoding = null;
          } else {
...
async function asArchive(content, req, flatten, fileName, isLegacyStyle) {
  const archive = await asRawArchive(content, req, fileName, isLegacyStyle);
  ...
  if (flatten !== 'disable' && (archive.options.flatten || flatten)) {
    const flatContent = await flattener.inline(content);
...
async function asRawArchive(content, req, fileName, isLegacyStyle) {
  ...
  if (isArchive(content)) {
    ...
    const zipContent = await outZip.generateAsync({type: 'nodebuffer', compression: 'DEFLATE'});
    return {
      style: 'ZIP',
      content: zipContent,
      options: {},
...
async function addFileToArchive(archive, fileName, buffer) {
  if (archive.style === 'LEGACY') {
    ...
    const outZip = await JSZip.loadAsync(archive.content);
...
async function loadFiles(legacyArchive, req, files) {
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    const encoding = determineEncoding(content);
    wsiEncodingCheck(encoding, fileName, req);
    const decodedContent = decode(content, encoding);
    const dom = d.loadSafeDOM(decodedContent, req, fileName);
    const locations = d.getLocations(dom, fileName, req).all;
  ...
  const newLocations = _.pullAll(_.uniq(_.flatten(allLocations)), knownFiles);
  if (newLocations.length === 0) {
    return legacyArchive;
  } else {
    // Load the new locations content
    const newLocationsContent = [];
    ...
    for (let i = 0; i < newLocationsContent.length; i++) {
      const content = encode(newLocationsContent[i].content, newLocationsContent.encoding);
      const loaded = {
...
const ACCEPTABLE = ['utf8', 'utf16', 'utf-8', 'utf-16'];
...
async function pipeArchiveZip(inputArchive, req, ignore, process) {
  ...
  let readCount = 0;
  let writeCount = 0;
  const READ_LIMIT = 64000000;
  const WRITE_LIMIT = 64000000;
  try {
    if (isZip(input)) {
      ...
      yauzl.fromBuffer(input, {
        ...
        if (err) {
          ...
          if (err.message.indexOf('relative path') >= 0) {
            ...
            const detail = g.http(u.r(req)).f('The content may have been uploaded with a wrong encoding (i.e. utf8). ' +
          ...
          zipFile.on('end', async () => {
            ...
            const content = await outZip.generateAsync({type: 'nodebuffer', compression: 'DEFLATE'});
            data.archive = {
              style: 'ZIP',
              content: content,
              ...
              zipFile.openReadStream(entry, function(err, readStream) {
                if (err) {
                  ...
                  readStream.on('end', function() {
                    try {
                      const fileContent = Buffer.concat(strings);
                      strings.length = 0;
                      ...
                      const pInfo = _processFile(fileName, fileContent, req, process);
...
function _processFile(fileName, encodedContent, req, process) {
  ...
  try {
    ...
    if (process) {
      const encoding = determineEncoding(encodedContent, fileName, req);
      wsiEncodingCheck(encoding, fileName, req);
      const decodedContent = decode(encodedContent, encoding);
      const processInfo = process(fileName, decodedContent.toString(), req, encoding);
      if (processInfo) {
        info.ignore = false;
...
function pipeArchiveLegacy(inputArchive, req, ignore, process) {
  ...
  const list = inputArchive.content;
  for (let i = 0; i < list.length; i++) {
    const fileName = list[i].fileName;
    const fileContent = list[i].content;
    const info = {
      fileName: fileName,
      ignore: false,
      error: null,
...
function isZip(content) {
  const start = content.toString('utf8', 0, 4).substr(0, 4);
  return (start === 'PK\u0003\u0004');
}
...
function isSymbolicLink(mode) {
  ...
  const isSymlink = isFile && (mode & 0o020000);
  return isSymlink;
}
...
function isExecutable(mode) {
  ...
  const isFile = (mode & 0o100000);
  const isExecutableFile = isFile && (mode & 0o000111);
...
function determineEncoding(content, filename, req) {
  ...
  if (Buffer.isBuffer(content)) {
    ...
    if (content.length > bomLength) {
      const newBuf = new Buffer.alloc(128);
      try {
        content.copy(newBuf, 0, bomLength, 128);
        const xmlSig = decode(newBuf, ret).toString();
        const index = xmlSig.indexOf('<?xml');
        if (index != -1) {
          const endIndex = xmlSig.indexOf('?>');
          if (endIndex != -1) {
            const realSig = xmlSig.substring(index, endIndex);
            const encIndex = realSig.indexOf('encoding=');
            if (encIndex != -1) {
              const quote = realSig.substr(encIndex + 9, 1);
              const endQuote = realSig.indexOf(quote, encIndex + 10);
              if (endQuote != -1) {
                let encoding = realSig.substring(encIndex + 10, endQuote);
                if (isUTF8(encoding)) {
                  encoding = 'utf8';
                }
        ...
        const errMsg = g.http(u.r(req)).f('An error (%s) was caught while processing file %s.', e.message, filename);
...
function decode(content, _encoding) {
  return content;
}

// Encode the content with the indicated encoding
function encode(content, _encoding) {
  return content;
}
...
function normalizeLocation(location, currLocation, stopAtRoot) {
  const newLocation = currLocation ? currLocation + '/' + location : location;
  let ret = newLocation;
  if (newLocation) {
...
function lastPathSeparator(path) {
  ...
  const lastBSlash = path.lastIndexOf('\\');
