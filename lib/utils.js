    ...
    if (typeof documentation !== 'string') {
      ...
      for (i = 0; i < documentation.length; i++) {
        if (!documentation[i]) {
          ...
          const value = _.trim(documentation[i]);
          if (value.length > 0) {
...
function makeValidationErr(message) {
  const valError = new Error(message);
...
function addValidationErr(valError, stringOrErr, req) {
  ...
  try {
    ...
    const subText = g.http(_(req)).f('An error occurred while parsing "%s".\n', 'MEMORY');
...
function findRefs(obj) {
  ...
  traverse(obj, function(curr, path, stack) {
    ...
    const isAllOf = path.length > 2 ? path[path.length - 3] === 'allOf' : false;
    const isAncRef = path.length > 1 ? path[path.length - 2] === 'x-anc-ref' : false;
...
function countKeys(obj, ignoreXML) {
  const localObj = ignoreXML ? deepCloneWithIgnoreKeys(obj, ['xml']) : obj;
  let count = 0;
...
function shareObjects(obj) {
  ...
  traverse(obj, function(curr) {
    if (curr && typeof curr === 'object') {
      ...
      if (curr.xml) {
        const cacheKey = JSON.stringify(curr.xml);
        if (map[cacheKey]) {
...
function traverseSchemaObjects(swagger, postfixFunction, prefixFunction) {
  let nsName;
  const context = {isRoot: false, inXSO: false, inXSOorSO: false};
    ...
    if (curr) {
      ...
      if (context.isRoot) {
        if (isSchemaObject(path, context.isRoot, curr)) {
          ...
          const index = (key2 === 'definitions') ? stack.length - 2 : stack.length - 3;
...
function shortenAllOfs(swagger) {
  ...
  traverseSchemaObjects(swagger, function(xso, nsName) {
    if (xso.allOf && xso.allOf.length > 2) {
      const slice = _.slice(xso.allOf, 1);
...
function squashAllOf(xso) {
  if (xso.allOf) {
    ...
    if (xso.allOf.length == 1 && !xso.allOf[0].$ref && !xso.allOf[0].items) {
      const target = deepClone(xso);
      const source = deepClone(xso.allOf[0]);
      delete target.allOf;
      delete source.xml;
...
function mergeXSO(target, sources) {
  ...
  for (let i = 0; i < sources.length; i++) {
    const source = deepClone(sources[i]);
    ...
    if (source.properties) {
      for (const propName in source.properties) {
  ...
  const xsoPropertiesCount = xso.properties ? Object.keys(xso.properties).length : 0;
...
function inPolyHierarchy(definitions, nsName, anc, map) {
  ...
  if (def && def['x-ibm-discriminator']) {
    const ref = '#/definitions/' + nsName;
    const mapRef = map.refs[ref];
    if (mapRef && (mapRef.count - mapRef.allOfCount > 0)) {
      return true;
    }
  }
...
function getSubTypes(definitions, subTypes) {
  ...
  for (const nsName in definitions) {
    ...
    if (def.allOf && def.allOf.length > 0 && def.allOf[0].$ref) {
      const base = getDefNameFromRef(def.allOf[0].$ref);
    ...
    if (def.discriminator) {
      if (def.oneOf && def.oneOf.length > 1) {
        ...
        for (let i = 1; i < def.oneOf.length; i++) {
          const nsName2 = getDefNameFromRef(def.oneOf[i].$ref);
...
function getAncestorRefs(definitions, nsName, req, anc) {
  ...
  if (def && def.allOf && def.allOf.length > 0 && def.allOf[0]['$ref']) {
    ...
    const ancNSName = getDefNameFromRef(ref);
    const ancDef = definitions[ancNSName];
...
function checkGateway(swagger, req) {
  ...
  const gateway = swagger['x-ibm-configuration'].gateway;
  ...
  if (swagger['x-ibm-configuration'].targets) {
    for (const service in swagger['x-ibm-configuration'].targets) {
      const inner = swagger['x-ibm-configuration'].targets[service];
...
function checkV5Policies(swagger, req) {
  const assembly = swagger['x-ibm-configuration'].assembly;
  const gateway = swagger['x-ibm-configuration'].gateway;
    ...
    if (name === 'execute') {
      ...
      for (let i = 0; i < execute.length; i++) {
        if (execute[i].invoke) {
          ...
          let input;
          for (const key in x.map.inputs) {
            const v = x.map.inputs[key].variable;
            if (v === 'request.body') {
              input = 'request';
          ...
          if (x.switch.case) {
            for (let i = 0; i < x.switch.case.length; i++) {
              const operations = x.switch.case[i].operations;
              x.switch.case[i] = {
                condition: '',
...
function addValidatePolicies(swagger, req) {
  const isAPIGW = swagger['x-ibm-configuration'].gateway == 'datapower-api-gateway';
...
function addTestPaths(swagger, req) {
  ...
  const service = Object.keys(swagger['x-ibm-configuration'].targets)[0];
  const targetSwagger = swagger['x-ibm-configuration'].targets[service];
  ...
  paths.forEach((path) => {
    ...
    if (swagger.paths[path].post &&
      ...
      const echoPath = path + '_TEST_InputEcho';
      const restRef = swagger.paths[path].post.parameters[0].schema.$ref;
      const soapRef = targetSwagger.paths[path].post.parameters[0].schema.$ref;
      const operationElement = Object.keys(getRefXSO(swagger, soapRef).properties['Envelope'].properties['Body'].properties)[0];
      ...
      const caseStmt = swagger['x-ibm-configuration'].assembly.execute[0]['operation-switch'].case;
    ...
    if (swagger.paths[path].post &&
      ...
      const code = swagger.paths[path].post.responses['200'] ? '200' : 'default';
      const targetCode = targetSwagger.paths[path].post.responses['200'] ? '200' : 'default';
      ...
      if (swagger.paths[path].post.responses[code] &&
        ...
        const echoPath = path + '_TEST_OutputEcho';
        const restRef = swagger.paths[path].post.responses[code].schema.$ref;
        const soapRef = targetSwagger.paths[path].post.responses[targetCode].schema.$ref;
        const operationElement = Object.keys(getRefXSO(swagger, soapRef).properties['Envelope'].properties['Body'].properties)[0];
...
function addGetMethods(swagger, req) {
  // Get the paths that can be optimized
  const paths = getPathsForGetOptimization(swagger);
  // Process each of the paths
  paths.forEach((path) => {
    ...
    const xso = getRefXSO(swagger, swagger.paths[path].post.parameters[0].schema.$ref);
    ...
    if (Object.keys(xso.properties).length > 0) {
      ...
      for (const key in xso.properties) {
        const prop = deepClone(xso.properties[key]);
        const param = {
          name: key,
          in: 'query',
          type: prop.type,
    ...
    if (cases) {
      ...
      // Create a GET case
      if (postCase) {
        const getCase = deepClone(postCase);
        const map = getCase.execute[0].map;
        if (map) {
          ...
          map.actions = [];
          const postActions = postCase.execute[0].map.actions;
          let base;
...
function getPathsForGetOptimization(swagger) {
  ...
  for (const path in swagger.paths) {
    ...
    if (swagger.paths[path].post &&
            swagger.paths[path].post.parameters &&
            swagger.paths[path].post.parameters.length == 1) {
      const parameter = swagger.paths[path].post.parameters[0];
      ...
      if (parameter.in === 'body' && parameter.schema && parameter.schema.$ref) {
        ...
        const xso = getRefXSO(swagger, parameter.schema.$ref);
        if (xso &&
                    xso.type === 'object' &&
                    xso.properties &&
...
function makeOld(swagger) {
  if (swagger['x-ibm-configuration'].targets) {
    for (const target in swagger['x-ibm-configuration'].targets) {
...
function _makeOld(swaggerNew, path) {
  const new2old = getOldNCNames(swaggerNew);
  const swaggerOld = deepClone(swaggerNew);
  swaggerOld.definitions = {};
  for (const newNCName in swaggerNew.definitions) {
    const oldNCName = new2old[newNCName];
...
function getOldNCNames(swagger) {
  const old2new = {};
  const new2old = {};
  for (const ncName in swagger.definitions) {
  ...
  for (const ncName in swagger.definitions) {
    ...
    if (words.length > 1 && words[1] === 'type') {
      ...
      if (old2new[ncNameOld]) {
        const def = swagger.definitions[ncName];
...
function getXSITypes(swagger, req) {
  ...
  const refs = findRefs(definitions);
  subTypes = getSubTypes(definitions, subTypes);
  subTypes = getSubTypesV3(definitions, subTypes);
  const desc = {};
  for (const nsName in definitions) {
    const descTypes = getDescendents(nsName, subTypes);
    if (descTypes && descTypes.length > 0) {
      desc[nsName] = descTypes;
    }
  }
  const map = {};
  for (const nsName in desc) {
    const ref = swagger.definitions ? '#/definitions/' + nsName : '#/components/schemas/' + nsName;
    if (!refs.refs[ref] || ((refs.refs[ref].count - refs.refs[ref].allOfCount) <= 0)) {
      continue;
    }
    const xso = definitions[nsName];
    if (!xso.discriminator && !xso['x-ibm-discriminator']) {
      continue;
    }
    const xsiType = getXSIType(xso, getDefinitions(swagger));
    const descs = desc[nsName];
    const xsiTypes = [];
    for (let i = 0; i < descs.length; i++) {
      const xso2 = definitions[descs[i]];
      const xsiType2 = getXSIType(xso2, getDefinitions(swagger));
      if (xsiType2 === 'NONE') {
...
function isNullable(xso, swagger) {
  if (xso.nullable !== undefined) {
    ...
    const nsName = getDefNameFromRef(xso['$ref']);
    const xso2 = getDefinitions(swagger)[nsName];
...
function getChildren(xso, swagger) {
  ...
  if (xso['$ref']) {
    ...
    const list = xso.allOf || xso.anyOf || xso.oneOf;
    const length = xso.discriminator ? 1 : list.length;
    for (let i = 0; i < length; i++) {
...
function oldNSName2newNSName(oldName, newSwagger, preferType) {
  ...
  const newDefinitions = getDefinitions(newSwagger);
  if (newDefinitions[oldName]) {
    return oldName;
  }
  const names = _.split(oldName, '_for_');
  if (names.length > 2) {
    return null;
  }
  let suffix = '';
  if (names.length == 2) {
    const words = _.split(names[1], '_');
    if (words > 1 && words[1] != 'element') {
      words.splice(1, 0, 'element');
    }
    suffix = '_for_' + _.join(words, '_');
  }
  const kind = ['element', 'typedef', 'type', 'attribute', 'group', 'attributeGroup', 'substitutionGroup'];
  const nsName = names[0];
  ...
  if (words.length > 1 && (kind.indexOf(words[1]) < 0)) {
    ...
    const nsNameType = _.join(words, '_');
    words[1] = 'typedef';
    const nsNameTypeDef = _.join(words, '_');
    words[1] = 'element';
    const nsNameElement = _.join(words, '_');
