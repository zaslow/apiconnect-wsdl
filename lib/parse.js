...
async function getJsonForWSDL(location, auth, options) {
  let outArchive; let outContent; // archive (zip) or content (file)
  options = options || {
  ...
  try {
    ...
    const allWSDLs = await postParse.merge(out.files, auth, options);
...
function sanityCheckDefinition(definitions, req) {
  ...
  const portTypes = u.makeSureItsAnArray(definitions.portType);
  const messages = u.makeSureItsAnArray(definitions.message);
  const bindings = u.makeSureItsAnArray(definitions.binding);
  const services = u.makeSureItsAnArray(definitions.service);
  ...
  for (let i = 0; i < portTypes.length; i++) {
    ...
    for (let j = 0; j < operations.length; j++) {
      ...
      const opName = operation['undefined'].name;
      const children = ['input', 'output', 'fault'];
      for (let c = 0; c < children.length; c++) {
        const key = children[c];
        if (operation[key] && operation[key]['undefined'] && operation[key]['undefined'].message) {
          const message = u.stripNamespace(operation[key]['undefined'].message);
          let found = false;
  ...
  for (let i = 0; i < bindings.length; i++) {
    ...
    const bindingName = binding['undefined'].name;
    const bOperations = u.makeSureItsAnArray(binding.operation);
    const portTypeName = u.stripNamespace(binding['undefined'].type);
    let portTypeFound = false;
    for (let j = 0; j < portTypes.length; j++) {
      const portType = portTypes[j];
      if (portType['undefined'].name === portTypeName) {
        portTypeFound = true;
        const pOperations = u.makeSureItsAnArray(portType.operation);
        ...
        for (let k = 0; k < bOperations.length; k++) {
          const bOpName = bOperations[k]['undefined'].name;
          let found = false;
          for (let l = 0; l < pOperations.length && !found; l++) {
            const pOpName = pOperations[l]['undefined'].name;
            if (pOpName === bOpName) {
              found = true;
            }
          }
  ...
  for (let i = 0; i < services.length; i++) {
    ...
    const serviceName = service['undefined'].name;
    const ports = u.makeSureItsAnArray(service.port);
...
* @param options
*   req: request or null (used for i18n negotiation)
* @return data object that contains the names of the services, portTypes, bindings, serviceOperations
*/
function getWSDLServices(allWSDLs, options) {
  const serviceData = getWSDLServicesAll(allWSDLs, options);
...
function getWSDLServicesAll(allWSDLs, options) {
  ...
  try {
    const wLen = allWSDLs.length;
    let operations; let operation; let ops;
    const serviceMap = {};
    for (let x = 0; x < wLen; x++) {
      const wsdlJson = allWSDLs[x].json;

      const portTypes = u.makeSureItsAnArray(wsdlJson.definitions.portType);
      const typeLen = portTypes.length;
      ...
      const bindings = u.makeSureItsAnArray(wsdlJson.definitions.binding);
      const binLen = bindings.length;
      for (let j = 0; j < binLen; j++) {
        ...
        for (let n = 0; n < operLen; n++) {
          operation = operations[n];
          ops.push(operation['undefined'].name);
          const opStyle = operation.operation && operation.operation['undefined'] ? operation.operation['undefined'].style : '';
          if (!style && opStyle) {
      ...
      const bindingNames = Object.keys(data.bindings);
      ...
      const services = u.makeSureItsAnArray(wsdlJson.definitions.service);
      const servLen = services.length;
      for (let k = 0; k < servLen; k++) {
        ...
        for (let l = 0; l < portLen; l++) {
          const port = ports[l];
          const portName = port['undefined'].name;
          let obj;
          if (l == 0) {
          ...
          const type = data.bindings[obj.bindingName] ? data.bindings[obj.bindingName].type : null;
          obj.operations = [];
          if (type) {
            const ops = data.portTypes[type];
            if (ops) {
        ...
        if (allWSDLs[x].serviceJSON && allWSDLs[x].serviceJSON != {}) {
          ...
          let found = false;
          for (let ss = 0; !found && ss < serviceMap[serv.service].length; ss++) {
            const s = serviceMap[serv.service][ss];
      ...
      for (const bindingName in data.bindings) {
    ...
    for (const s in serviceMap) {
      if (serviceMap[s].length > 1) {
        for (let z = 0; z < serviceMap[s].length; z++) {
...
function onlySoapPorts(ports, bindings) {
  ...
  for (let i = 0; i < portLen; i++) {
    if (ports[i].address && ports[i]['undefined'] && ports[i]['undefined'].binding) {
      const bindingName = u.stripNamespace(ports[i]['undefined'].binding);
...
* @return WSDLEntry object for the indicated service.
*/
function findWSDLForServiceName(allWSDLs, serviceName, serviceFilename) {
  let ret = null;
  const len = allWSDLs.length;
  ...
  let slugifyServiceFullName;
  const mangleIndex = serviceName.indexOf('-from-');
  if (mangleIndex > 0) {
...
async function parse(inFileName, auth, options) {
  ...
  const out = await fileUtils.asContent(inFileName, inFileName, auth, null, req);
  const rawContent = out.content;
  const isZip = fileUtils.isZip(out.content);
  ...
  const isLegacy = out.fileName && !options.selfContained;
  let fileName = null;
  if (isLegacy) {
    fileName = inFileName; // Use the full path
  } else {
    if (out.fileName) {
      const i = fileUtils.lastPathSeparator(out.fileName);
      fileName = i < 0 ? out.fileName : out.fileName.substr(i + 1); // Get the short name
    }
  }
  // Convert the content into an archive
  const archive = await getArchive(out.content, req, options.flatten, fileName, isLegacy);
  ...
  const fileSet = await parsePass1(archive, options);
  const result = await parsePass2(archive, fileSet, options);
...
async function parsePass1(archive, options, fileSet) {
  ...
  function processFile(fileName, content, req) {
    ...
    try {
      ...
      const dom = d.loadSafeDOM(content, req, fileName);
  ...
  try {
    ...
    if (options.implicitHeaderFiles) {
      for (let i = 0; i < options.implicitHeaderFiles.length; i++) {
        const fileName = options.implicitHeaderFiles[i];
        if (!fileSet[fileName]) {
    ...
    if (messages.length > 0) {
      ...
      const existingFiles = Object.keys(fileSet);
      ...
      const diff = _.difference(allFiles, existingFiles);
      let done = true;
...
function massageXMLParseMessage(message) {
  const pre = 'An XML parsing error was found.  Please ensure that the file is a valid XSD or WSDL file. ';
  let detail = '';
...
async function parsePass2(archive, fileSet, options) {
  ...
  function processFile(fileName, content, req) {
    ...
    try {
      ...
      const index = fileUtils.lastPathSeparator(fileName);
      if (index != -1) {
      ...
      if (fileEntry.error) {
        ...
        const checkWSDL = parseUtils.validateWSDLJson(fileEntry.json, fileEntry.filename);
        if (checkWSDL.valid) {
          fileEntries.push(fileEntry);
          fileEntry.type = 'wsdl';
          valid = true;
        ...
        const insert = fileEntry.filename === fileUtils.INTERNAL_WSDL ? '(input file)' : fileEntry.filename;
        const e = g.http(u.r(req)).Error('The content of file %s is not wsdl or xsd.  This file cannot be processed.  Please correct your input.', insert);
        if (fileUtils.isWSDL(fileName)) {
          throw e;
        } else {
          fileEntry.error = e;
  ...
  try {
    const out = await fileUtils.pipeArchive(archive, req, shouldIgnoreFile, processFile);
...
async function getArchive(content, req, flatten, fileName, isLegacy) {
  const archive = await fileUtils.asArchive(content, req, flatten, fileName, isLegacy);
  const buffer = fs.readFileSync(__dirname + '/files/XMLSchema.xsd');
...
async function sanitize(content, req) {
  const isZip = fileUtils.isZip(content);
  if (!isZip) {
    if (typeof content === 'string') {
      return sanitizeFile(content, req);
    } else {
      const encoding = fileUtils.determineEncoding(content);
      const decodedContent = fileUtils.decode(content, encoding);
      return fileUtils.encode(sanitizeFile(decodedContent, req), encoding);
    }
  } else {
    const out = await fileUtils.asContent(content, content, null, null, req);
...
function sanitizeFile(content, req, fileName) {
  ...
  const dom = d.loadSafeDOM(content);
  d.sanitizeDOM(dom);
