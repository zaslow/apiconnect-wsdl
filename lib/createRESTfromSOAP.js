...
function migrateFromV5(swaggerV5, options) {
  const swaggerV6 = u.deepClone(swaggerV5);
  return swaggerV6;
}
...
function migrateToV5(swaggerV6, options) {
  const swaggerV5 = u.deepClone(swaggerV6);
...
function getSwaggerForSOAPREST(soapSwagger, createOptions) {
  ...
  const service = soapSwagger['x-ibm-configuration']['wsdl-definition'] ?
        soapSwagger['x-ibm-configuration']['wsdl-definition'].service :
        soapSwagger.info['x-ibm-name'];

  const numPaths = soapSwagger.paths ? Object.keys(soapSwagger.paths).length : 0;
...
function initRESTSwagger(service, type, wsdlDef) {
  const ibmName = u.slugifyName(service);
  const swagger = {
    'swagger': '2.0',
    'info': {
      'x-ibm-name': ibmName,
...
function addTargetService(soapSwagger, restSwagger) {
  ...
  const srcRef = '#/definitions/';
  const tgtRef = '#/x-ibm-configuration/targets/' + service + '/definitions/';
...
function addRESTPaths(soapSwagger, restSwagger) {
  for (const soapPath in soapSwagger.paths) {
...
function addAssemblySwitch(restSwagger, service, options) {
  ...
  const soapDefPath = '#/x-ibm-configuration/targets/' + service + '/definitions/';
  const isSOAP12 = restSwagger['x-ibm-configuration']['wsdl-definition']['soap-version'] === '1.2';

  for (const pathKey in restSwagger['x-ibm-configuration'].targets[service].paths) {
    const name = pathKey.substring(1);
    const path = restSwagger['x-ibm-configuration'].targets[service].paths[pathKey];
    const proxyInvoke =
    ...
    const soapProxyURL = proxyInvoke['target-url'];
    ...
    const soapOutputRef = soapDefPath + name + 'Output';
...
function createOperationDesc(soapSwagger, options) {
  ...
  const s = jsonifySwagger(soapSwagger, options.jsonStyle, options.req);
  for (const soapPath in s.paths) {
    const opName = soapPath.substring(1);
...
function createOperationDescInfo(s, name, options) {
  const isFault = !name.endsWith('Input') && !name.endsWith('Output');
  const soapDef = s.definitions[name];
  let info = {};
  if (soapDef) {
    info = {
      wrapped: false,
    ...
    const hdrProps = headerProperties(s, soapDef, options.req);
    const bodyProps = bodyProperties(s, soapDef);
...
function addRESTOperationDefinitions(operationDesc, restSwagger, service, options) {
  const isSOAP12 = restSwagger['x-ibm-configuration']['wsdl-definition']['soap-version'] === '1.2';
  for (const opKey in operationDesc) {
    let ref;
    const op = operationDesc[opKey];
    if (op.request.def) {
    ...
    if (options.mapSOAPFaults) {
      if (op.fault.def) {
        ...
        const opFault = getDef(restSwagger, op.fault.def, options.req);
        ...
        const target = restSwagger['x-ibm-configuration'].targets[service];
...
function addRESTDefinitions(soapSwagger, restSwagger, options) {
  ...
  for (const nsName in s.definitions) {
    if (!ignore[nsName]) {
...
function jsonifySwagger(soapSwagger, jsonStyle, req) {
  const s = renameAttributes(soapSwagger, soapSwagger, req);
...
function badgerfishDefinitions(swagger) {
  return u.traverseSchemaObjects(swagger, function(xso, nsName, context, path) {
    ...
    if (key && typeof key === 'string' && key.startsWith('@')) {
      const format = getScalarFormat(xso);
      ...
      const format = getScalarFormat(xso);
      const newXSO = _.pick(xso, ['properties', 'nullable', 'x-nullable', 'description', 'required',
...
function addAssemblyCatchSOAPError(swagger, service, operationDesc, options) {
  const isSOAP12 = swagger['x-ibm-configuration']['wsdl-definition']['soap-version'] === '1.2';
  const textField = isSOAP12 ? 'Reason.Text' : 'faultstring';
    ...
    if (xso.properties) {
      ...
      for (const key in xso.properties) {
        ...
        if (key.startsWith('@')) {
          ...
          properties['@' + key] = prop;
          if (xso.required) {
            const i = _.indexOf(xso.required, key);
            if (i >= 0) {
              xso.required[i] = '@' + key;
            }
          }
...
function addAssemblyIO(restSwagger) {
  ...
  const opSwitch = restSwagger['x-ibm-configuration'].assembly.execute[0]['operation-switch'];
  for (let i = 0; i < opSwitch.case.length; i++) {
    const _case = opSwitch.case[i];
    const path = _case.operations[0].path;
    const inputMap = _case.execute[0].map;
    const outputMap = _case.execute[2].map;
...
function addAssemblyWiring(swagger, operationDesc, req) {
  // Find operation-switch
  const opSwitch = swagger['x-ibm-configuration'].assembly.execute[0]['operation-switch'];
  for (let i = 0; i < opSwitch.case.length; i++) {
    const _case = opSwitch.case[i];
    const path = _case.operations[0].path;
    const inputMap = _case.execute[0].map;
...
function isWrapped(path, operationDesc, io) {
  const opName = path.substring(1);
  const op = operationDesc[opName];
...
function getWireActions(swagger, inputs, outputs, req) {
  let inPath; let inDef; let outPath; let outDef;
  if (inputs.request) {
    inDef = getDef(swagger, inputs.request.schema, req);
    inPath = 'request';
  } else if (inputs.input) {
    const schema = getDef(swagger, inputs.input.schema, req);
    const body = schema.properties['Envelope'].properties['Body'];
    const name = getKey(body.properties);
  ...
  if (outputs.response) {
    ...
    const schema = getDef(swagger, outputs.body.schema, req);
    const body = schema.properties['Envelope'].properties['Body'];
    const name = getKey(body.properties);
...
function getWireActionsWrapped(swagger, inputs, outputs, req) {
  let actions = [];
  if (inputs.request) {
    const inDef = getDef(swagger, inputs.request.schema, req);
    const schema = getDef(swagger, outputs.body.schema, req);
    const body = schema.properties['Envelope'].properties['Body'];
    const header = schema.properties['Envelope'].properties['Header'];
    ...
    const outDef = getDef(swagger, outputs.response.schema, req);
    const schema = getDef(swagger, inputs.input.schema, req);
    const body = schema.properties['Envelope'].properties['Body'];
    const header = schema.properties['Envelope'].properties['Header'];
