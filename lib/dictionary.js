...
function buildDictionary(schemaList, detectedWSAddressing, namespaces, wsdlTNS, req) {
  ...
  for (let i = 0; i < schemaLen; i++) {
    ...
    const values = _.values(namespaces);
    if (schema.xmlns) {
      for (const prefix in schema.xmlns) {
        const ns = schema.xmlns[prefix];
  ...
  for (let i = 0; i < schemaLen; i++) {
    ...
    const schemaNamespaces = schema.wsdlXmlns ? schema.wsdlXmlns : namespaces;
    const implicitHeaderSchema = schema['undefined'].implicitHeaderSchema;
    ...
    const versionPrefix = u.getPrefixForNamespace(SCHEMA_VERSIONING_NS, schema.xmlns, true) ||
    ...
    if (schema.complexType) {
      const defComplexTypes = u.makeSureItsAnArray(schema.complexType);
      const complexLen = defComplexTypes.length;
      for (let j = 0; j < complexLen; j++) {
        const complexType = defComplexTypes[j];
        const nsName = resolveNsName(complexType['undefined'].name, 'typedef', schemaTNS, namespaces);
        const isAbstract = complexType['undefined'].abstract && complexType['undefined'].abstract.toLowerCase() === 'true';
        const tagInfo = {
          prefix: prefix,
        ...
        const refNSName = resolveNsName(complexType['undefined'].name, 'type', schemaTNS, namespaces);
    ...
    if (schema.simpleType) {
      const defSimpleTypes = u.makeSureItsAnArray(schema.simpleType);
      const simpleLen = defSimpleTypes.length;
      for (let j = 0; j < simpleLen; j++) {
        const simpleType = defSimpleTypes[j];
        const nsName = resolveNsName(simpleType['undefined'].name, 'typedef', schemaTNS, namespaces);
        const tagInfo = {
          prefix: prefix,
        ...
        if (simpleType.list) {
          ...
          const xsdPrefixes = getXSDPrefixes(namespaces);
          const stringType = xsdPrefixes.length > 0 ? xsdPrefixes[0] + ':string' : 'string';
          dictEntry[nsName].schema['undefined'].type = stringType;
        }
        const refNSName = resolveNsName(simpleType['undefined'].name, 'type', schemaTNS, namespaces);
        dictEntry[refNSName] = u.deepClone(dictEntry[nsName]);
        dictEntry[refNSName].schemaType = 'typeOf';
        dictEntry[refNSName].typeNSName = nsName;
        dictEntry[refNSName].for = 'type';
    ...
    if (schema.element) {
      ...
      const defElements = u.makeSureItsAnArray(schema.element);
      const elemLen = defElements.length;
      for (let j = 0; j < elemLen; j++) {
        const element = defElements[j];
        const elemName = element['undefined'].name;
        const nsName = resolveNsName(elemName, 'element', schemaTNS, namespaces);
        ...
        const isAbstract = element['undefined'].abstract && element['undefined'].abstract.toLowerCase() === 'true';
        const isNillable = element['undefined'].nillable && element['undefined'].nillable.toLowerCase() === 'true';
        const defaultValue = element['undefined'].default || element['undefined'].final;
        ...
        if (!('complexType' in element) &&
          ...
          const isXSD = isXSDType(element['undefined'].type, namespaces, schema.xmlns);
          const isAnyType = isXSDAnyType(element['undefined'].type, namespaces, schema.xmlns);
          ...
          if (isAnyType) {
            ...
            const elemType = resolveNameInNamespace(element['undefined'].type, 'type', schema.xmlns, namespaces, schemaTNS);
            if (!dictEntry[nsName]) {
              dictEntry[nsName] = {
                schema: element,
                tagInfo: tagInfo,
        ...
        if (element['undefined'].substitutionGroup) {
          ...
          const groupGroupNS = resolveNameInNamespace(element['undefined'].substitutionGroup, 'element', schema.xmlns, namespaces, schemaTNS);
    ...
    if (schema.group) {
      ...
      const defGroups = u.makeSureItsAnArray(schema.group);
      const gpLen = defGroups.length;
      for (let j = 0; j < gpLen; j++) {
        const groupType = defGroups[j];
        const nsName = resolveNsName(groupType['undefined'].name, 'group', schemaTNS, namespaces);
        if (!dictEntry[nsName]) {
          dictEntry[nsName] = {
            schema: groupType,
            tagInfo: {
    ...
    if (schema.attributeGroup) {
      ...
      // the constructs in the group.
      const defAttrGroups = u.makeSureItsAnArray(schema.attributeGroup);
      for (let j = 0; j < defAttrGroups.length; j++) {
        const attrGroupType = defAttrGroups[j];
        const nsName = resolveNsName(attrGroupType['undefined'].name, 'attributeGroup', schemaTNS, namespaces);
        if (!dictEntry[nsName]) {
          dictEntry[nsName] = {
    ...
    if (schema.attribute) {
      ...
      const attrs = u.makeSureItsAnArray(schema.attribute);
      for (let j = 0; j < attrs.length; j++) {
        const attr = attrs[j];
        const attrName = attr['undefined'].name;
        const nsName = resolveNsName(attrName, 'attribute', schemaTNS, namespaces);
        schemaAttributes[schemaTNS].push(nsName);

        const defaultValue = attr['undefined'].default || attr['undefined'].final;
        let tagInfo;
        if (nsName) {
          tagInfo = {
            xml: {
        ...
        if (attr.simpleType) {
          ...
          const isAttrXSD = isXSDType(attr['undefined'].type, namespaces, schema.xmlns);
          ...
          if (isAttrXSD) {
            ...
            const attrType = resolveNameInNamespace(attr['undefined'].type, 'type', schema.xmlns, namespaces, schemaTNS);
            if (attrType) {
              if (!dictEntry[nsName]) {
                dictEntry[nsName] = {
                  schema: attr,
  ...
  for (const nsName in substitutions) {
    const dictEntry = dict.dictEntry[nsName];
    if (dictEntry) {
      const list = [];
      getSubstitutions(substitutions, list, nsName);
      const subGroupNSName = resolveNsName(dictEntry.tagInfo.name, 'substitutionGroup', dictEntry.tns, namespaces);
      const subGroupDictEntry = u.deepClone(dictEntry);
  ...
  for (const nsName in dict.dictEntry) {
    const dictEntry = dict.dictEntry[nsName];
    if (dictEntry.schema && dictEntry.for === 'typedef') {
      ...
      if (ext) {
        const baseType = bestMatch(ext['undefined'].base, 'typedef', dictEntry, dict, namespaces);
        if (baseType) {
...
function allDescendents(nsName, dictEntry, descendents) {
  ...
  const test = _.union(descendents, dictEntry[nsName].subTypes, _.isEqual);
  ...
  for (let i = 0; i < dictEntry[nsName].subTypes.length; i++) {
    const subType = dictEntry[nsName].subTypes[i];
...
function getSubstitutions(substitutions, list, nsName) {
  ...
  if (substitutions[nsName]) {
    const subList = substitutions[nsName];
...
function pruneSchemas(schemaList, req) {
  ...
  const schemaList3 = [];
  const chameleonSchemas = {};
  for (let i = 0; i < schemaList2.length; i++) {
    const schema = schemaList2[i];
  ...
  if (Object.keys(chameleonSchemas).length > 0) {
    const len = schemaList3.length;
    for (let i = 0; i < len; i++) {
      const schema = schemaList3[i];
      ...
      if (schema.include) {
        ...
        const fileNames = Object.keys(chameleons);
        ...
        for (const fileName in chameleons) {
          const newSchema = u.deepClone(chameleons[fileName]);
...
function getChamelionSchemas(allChameleons, schemaLocation, chameleons) {
  ...
  const fileName = u.fileNameFromPath(schemaLocation);
  if (allChameleons[fileName] && !chameleons[fileName]) {
    const schema = allChameleons[fileName];
...
function resolveNsName(name, kind, tns, namespaces) {
  ...
  if (tns) {
    const prefix = u.getPrefixForNamespace(tns, namespaces);
    if (prefix && (prefix !== '__tns__')) {
      ret += '_' + prefix;
    }
  }
...
function checkKind(kind) {
  const allowed = ['wsdl', 'element', 'typedef', 'type', 'attribute', 'group', 'attributeGroup', 'substitutionGroup'];
  if (!kind) {
...
function resolveNameInNamespace(name, kind, localNamespaces, globalNamespaces, tns, dict) {
  ...
  if (name) {
    ...
    if (index != -1) {
      const rawPrefix = name.substring(0, index);
      ...
      if (dict) {
        const prefixes = preferedOrderPrefixes(localNamespaces, globalNamespaces, tns);
