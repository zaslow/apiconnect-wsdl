...
function generateSwaggerForWsdlProxy(wsdlEntry, serviceName, wsdlId, createOptions) {
  ...
  const openapi = getSwaggerForService(wsdlEntry, serviceName, wsdlId, createOptions);
  if (createOptions.defaults) {
    const defaults = u.deepClone(createOptions.defaults);
...
function generateSwaggerForSoapToRest(wsdlEntry, serviceName, wsdlId, createOptions) {
  createOptions = copts.validate(createOptions);
  const openapi_soap = getSwaggerForService(wsdlEntry, serviceName, wsdlId, createOptions);
  ...
  const openapi_rest = rest.getSwaggerForSOAPREST(openapi_soap, createOptions);
  if (createOptions.defaults) {
    const defaults = u.deepClone(createOptions.defaults);
    // mergeWith is not available in lodash^3.10
    // so do a merge and assign
...
function getSwaggerForService(wsdlEntry, serviceName, wsdlId, createOptions) {
  ...
  const globalNamespaces = wsdlEntry.namespaces;
  const wsdlDefNamespaces = u.deepClone(wsdlEntry.namespaces);
  ...
  const pathCount = swagger.paths ? Object.keys(swagger.paths).length : 0;
  ...
  const keyCount = u.countKeys(swagger, true);
  ...
  const messages = R.getMessages(req, createOptions.level);
  if (messages) {
...
function addXSDElementReferences(refMap, dict) {
  ...
  for (const nsName in dict.dictEntry) {
    ...
    const originalName = dictEntry.tagInfo.name;
...
function addHeaderDefinitionsForXSD(swagger, dict) {
  ...
  for (const nsName in dict.dictEntry) {
    ...
    if (dictEntry.tagInfo) {
      const elementNSName = nsName + '_of_' + dictEntry.tagInfo.name;
      if (swagger.definitions[elementNSName]) {
        // Get the original name of the element (not the mangled definition name)
        const origElementName = swagger.definitions[elementNSName].xml.name;
        const headerNsName = nsName + '_Header';
