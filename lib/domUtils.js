...
function getKnownNamespaces() {
  const known = [];
  for (const ns in NAMESPACE_INFO) {
...
function traverseDOM(node, f, list, postfix) {
  ...
  if (node && node.childNodes) {
    const isSchemaOrDef = (node.localName === 'schema' || node.localName === 'definitions');
    if (isSchemaOrDef) {
      list.push(node);
    }
...
function getLocations(doc, fileName, req) {
  ...
  if (fileName) {
    ...
    const lastBSlash = fileName.lastIndexOf('\\');
    const i = lastSlash >= lastBSlash ? lastSlash : lastBSlash;
  ...
  for (let i = 0; i < e.length; i++) {
    const newLocation = e[i].getAttribute('schemaLocation');
    if (newLocation) {
      const location = fileUtils.normalizeLocation(newLocation, currLocation, true);
  ...
  for (let i = 0; i < e.length; i++) {
    const newLocation = e[i].getAttribute('location');
    if (newLocation) {
      const location = fileUtils.normalizeLocation(newLocation, currLocation, true);
      if (locations.all.indexOf(location) < 0) {
        locations.all.push(location);
...
function promoteNamespaceDeclarations(dom) {
  ...
  function processNode(node) {
    if (node.nodeType === ELEMENT_NODE) {
      const isSchema = node.localName === 'schema' && node.namespaceURI === SCHEMA_NS;
      const nsMap = getNSMap(node);
      if (isSchema) {
        ...
        const schemaNSMap = _.last(schemaInfo).nsMap;
        const updateMap = {};
        ...
        for (const prefix in nsMap) {
          if (!schemaNSMap[prefix]) {
            ...
            let existingPrefix;
            for (const p in schemaNSMap) {
              if (schemaNSMap[p] === nsMap[prefix]) {
                existingPrefix = p;
                break;
              }
            }
            const newPrefix = existingPrefix || CONFLICT_PREFIX + inner++;
        ...
        if (updateNeeded) {
          const oldUpdateMap = updateInfo.length === 0 ? {} : u.deepClone(_.last(updateInfo).map);
          updateInfo.push({
            node: node,
  ...
  function postNode(node) {
    if (node.nodeType === ELEMENT_NODE) {
      ...
      if (isSchema) {
        ...
        for (const prefix in schemaNSMap) {
...
function changePrefixRefs(node, updateMap) {
  const attrNames = ['base', 'type', 'ref'];
  for (const oldPrefix in updateMap) {
    const newPrefix = updateMap[oldPrefix];
    if (oldPrefix === '') {
      ...
      attrNames.forEach(function(name) {
        ...
        if (value && value.startsWith(oldPrefix + ':')) {
          const newPrefixRef = newPrefix === '' ? '' : newPrefix + ':';
...
function getNamesMap(dom) {
  const map = {nodes: {}, attrs: {}, namespaces: {}, nsMap: {}};
  traverseDOM(dom, function(node) {
    ...
    if (node.nodeName) {
      const i = node.nodeName.indexOf(':');
      const name = i > -1 ? node.nodeName.substring(i + 1) : node.nodeName;
      map.nodes[name] = map.nodes[name] ? map.nodes[name]++ : 1;
    }
    if (node.attributes) {
      for (let i = 0; i < node.attributes.length; i++) {
        const attr = node.attributes[i].nodeName;

        if (attr.startsWith('xmlns:') || attr === 'xmlns') {
          const ns = node.attributes[i].nodeValue;
          map.namespaces[ns] = map.namespaces[ns] ? map.namespaces[ns]++ : 1;
          const prefix = attr === 'xmlns' ? '' : node.attributes[i].localName;
          const value = map.nsMap[prefix] || [];
        ...
        const name = index > -1 ? attr.substring(index + 1) : attr;
...
function removeXSDImportsAndIncludes(text, req) {
  const dom = loadSafeDOM(text, req);
  traverseDOM(dom, function(node) {
    if (node.nodeType === ELEMENT_NODE) {
    ...
    switch (node.nodeType) {
      ...
      case ELEMENT_NODE:
        ...
        if ((node.localName === 'appinfo' && node.namespaceURI === SCHEMA_NS) ||
          ...
          const info = getNamespaceInfo(node.namespaceURI);
              ...
              for (let i = 0; i < node.attributes.length;) {
                ...
                if (attr.nodeName && (attr.nodeName === 'xmlns' || attr.nodeName.startsWith('xmlns:'))) {
                  ...
                  if (attr.namespaceURI) {
                    const info = getNamespaceInfo(attr.namespaceURI);
                    if (!info.known || (!info.base && !info.extension)) {
                      attr = null;
                    }
                  }
...
function pruneAndAddID(dom, fileName, req) {
  ...
  traverseDOM(dom, function(node, list) {
    if (node.nodeType === ELEMENT_NODE) {
      const info = getNameInfo(node.localName);
...
function protectDocumentation(contents) {
  ...
  if (contents) {
    ...
    while ((result = regex.exec(contents))) {
      ...
      const name = prepend + 'documentation';
...
function protectDocSpecific(contents, elementName) {
  ...
  const elementStart = '<' + elementName;
  const elementEnd = '</' + elementName;
  while (offset < len) {
    ...
    if (index === -1) {
      ...
      const endStart = contents.indexOf('>', index);
      const endStartEnd = contents.indexOf('/>', index);
      if (endStartEnd > 0 && endStartEnd < endStart) {
        ...
        // <xs:documentation >
        ret += contents.substring(offset, endStart + 1);
        const endIndex = contents.indexOf(elementEnd, endStart);
        const endEnd = contents.indexOf('>', endIndex);
        let doc = contents.substring(endStart + 1, endIndex);
...
function syntaxCheck(dom, fileName, req) {
  ...
  traverseDOM(dom, function(node) {
    const token = getToken(node, req, fileName, soapBindingNS);
    ...
    if (parents) {
      const parentToken = getToken(node.parentNode, req, fileName, null);
      if (parents.indexOf(parentToken) < 0) {
        const msg = g.http(u.r(req)).f('Expected %s %s to have parent of %s but found %s. This invalid syntax was found in file %s.', token, getTokenName(node), parents, parentToken, fileName);
      ...
      const regexp = CHILDREN[token] || CHILDREN[parentToken + token];
      let childTokens = '';
      if (regexp) {
        childTokens = getChildTokens(node, req, fileName, soapBindingNS);
        if (!regexp.test(childTokens)) {
          const msg = g.http(u.r(req)).f('Expected %s %s to have children %s but found %s. This invalid syntax was found in file %s.', token, getTokenName(node), regexp, childTokens, fileName);
          // Mime syntax is a bit loose, so embed an error versus throwing a fatal error
          if (msg.includes('mime')) {
            R.error(req, msg);
          } else {
      ...
      if (token === '<xsd_restriction>') {
        const facets = ['length', 'pattern', 'totalDigits', 'fractionDigits', 'whiteSpace',
          'minLength', 'maxLength', 'minInclusive', 'maxInclusive', 'minExclusive', 'maxExclusive'];

        for (let i = 0; i < facets.length; i++) {
          const r = new RegExp('/' + facets[i] + '/g');
          const count = (childTokens.match(r) || []).length;
          if (count > 1) {
            const msg = g.http(u.r(req)).f('Found %s occurrences of facet %s. This invalid syntax was found in file %s.', count, facets[i], fileName);
            throw new Error(msg);
          }
        }
      }
  ...
  if (node.nodeType === DOCUMENT_NODE) {
    ...
    if (node.namespaceURI === SCHEMA_NS) {
      ...
      if (soapBindingNS && soapBindingNS !== node.namespaceURI) {
        const msg = g.http(u.r(req)).f('Found a SOAP namespace of \'%s\' on element \'%s\' which does not match the SOAP binding namespace \'%s\'. The invalid syntax was found in file %s.', node.namespaceURI, node.localName, soapBindingNS, fileName);
        R.error(req, msg);
      }
