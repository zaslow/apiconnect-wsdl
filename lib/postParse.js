...
async function merge(files, auth, options) {
    ...
    for (let j = 0; j < files.length; j++) {
      ...
      if (entry.type == 'wsdl') {
        if (entry.json.definitions && entry.json.definitions.types && entry.json.definitions.types.schema) {
          ...
          const slashes = (entry.fullName.match(/\//g) || []).length;
          let root = '';
  ...
  for (i = 0; i < len; i++) {
    ...
    if (entry.type == 'wsdl') {
      if (entry.json.definitions.import) {
        const imports = u.makeSureItsAnArray(entry.json.definitions.import);
        const impLen = imports.length;
        for (let j = 0; j < impLen; j++) {
          const imp = imports[j];
          const location = imp['undefined'].location;
          if (location) {
      ...
      if (options.apiFromXSD) {
        if (entry.json.schema.import) {
          const imports = u.makeSureItsAnArray(entry.json.schema.import);
          const impLen = imports.length;
          for (let j = 0; j < impLen; j++) {
            const imp = imports[j];
            const location = imp['undefined'].schemaLocation;
            if (location) {
              hasBeenImported[location] = true;
            }
          } // end for
        ...
        if (entry.json.schema.include) {
          const includes = u.makeSureItsAnArray(entry.json.schema.include);
          const incLen = includes.length;
          for (let j = 0; j < incLen; j++) {
            const imp = includes[j];
            const location = imp['undefined'].schemaLocation;
            if (location) {
              hasBeenImported[location] = true;
            }
  ...
  const rootLen = rootWSDLs.length;
  const impDefs = [];
  for (i = 0; i < rootLen; i++) {
    const entry = rootWSDLs[i];
    if (entry.type == 'wsdl') {
      impDefs.push(checkOneWSDLImport(entry, allWSDLs, files, auth, options));
    } else if (entry.type == 'xsd') {
      if (!entry.serviceJSON) {
        const name = (entry.filename === fileUtils.INTERNAL_WSDL) ?
...
async function expandWSDLImports(wsdlJson, baseFilename, files, mergeList, alreadySeen, refFile, auth, options) {
  if (wsdlJson.definitions.import) {
    const imports = u.makeSureItsAnArray(wsdlJson.definitions.import);
    const iLen = imports.length;
...
function setAbsoluteSchemaLocation(obj, basePath) {
  const location = obj && obj['undefined'] ? obj['undefined'].schemaLocation : null;
  if (location && (!(location.substr(0, 7) == 'http://' || location.substr(0, 8) == 'https://'))) {
    const fn = location.replace(/\\/g, '/');
...
function mergeWSDL(targetJson, sourceFile, targetFileNamespaces) {
  ...
  const sourceJson = sourceFile.json;
  let tns = '';
  ...
  const fn = sourceFile.fullName.replace(/\\/g, '/');
  let basePath = '';
      ...
      if (key == 'undefined') {
        ...
        if (targetJson.definitions[key].schema) {
          if (Array.isArray(targetJson.definitions[key].schema)) {
            const schemaLen = targetJson.definitions[key].schema.length;
            for (let k = 0; k < schemaLen; k++) {
              const schema = targetJson.definitions[key].schema[k];
        ...
        const keyLen = targetJson.definitions[key].length;
        for (let i = 0; i < keyLen; i++) {
          const item = targetJson.definitions[key][i];
      ...
      if (key != 'undefined') {
        if (key == 'types') {
          ...
          if (Array.isArray(sourceJson.definitions[key])) {
            const len = sourceJson.definitions[key].length;
            for (let j = 0; j < len; j++) {
              const srcItem = sourceJson.definitions[key][j];
              try {
            ...
            const def = sourceJson.definitions[key];
            try {
...
async function checkOneWSDLImport(rootWSDL, allWSDLs, files, auth, options) {
  const wsdlJson = rootWSDL.json;
  const fn = rootWSDL.fullName.replace(/\\/g, '/');
  let baseFilename = '';
  const index = fn.lastIndexOf('/');
  if (index != -1) {
  ...
  const mergeLen = mergeList.length;
  if (mergeLen > 0) {
    for (let j = 0; j < mergeLen; j++) {
      const mergeFile = mergeList[j];
...
async function checkOneXSDImport(rootDoc, allDocs, files, auth, options) {
  ...
  const fn = rootDoc.fullName.replace(/\\/g, '/');
  let baseFilename = '';
  const index = fn.lastIndexOf('/');
  if (index != -1) {
    baseFilename = fn.substring(0, index);
...
async function expandOneWSDLImport(imp, baseFilename, files, mergeList, alreadySeen, refFile, wsdlJson, auth, options) {
  ...
  const importNamespace = imp && imp['undefined'] ? imp['undefined'].namespace : null;
  if (location) {
        ...
        if (entry.fullName == location || entry.filename == location) {
          ...
          if (entry.type == 'xsd') {
            ...
            if (entry.json.schema['undefined']) {
              const tns = entry.json.schema['undefined'].targetNamespace;
...
async function expandSchemaImports(inSchema, baseFilename, files, mergeList, alreadySeen, refFile, auth, isWSDLTypesSchema, options) {
  const schemaList = u.makeSureItsAnArray(inSchema);
  const schemaLen = schemaList.length;
  for (let i = 0; i < schemaLen; i++) {
    const schema = schemaList[i];
    let tns = '';
    if (schema['undefined']) {
      const targns = schema['undefined'].targetNamespace;
      if (targns) {
        tns = targns;
      }
    }
    ...
    if (includes) {
      ...
      const inLen = includes.length;
      for (let k = 0; k < inLen; k++) {
        const impIn = includes[k];
...
async function checkForSchemaImports(wsdlJson, wsdlFilename, files, refFile, auth, options) {
  const fn = wsdlFilename.replace(/\\/g, '/');
  let baseFilename = '';
  const index = fn.lastIndexOf('/');
  if (index != -1) {
    baseFilename = fn.substring(0, index);
  ...
  if (wsdlJson.definitions.types && wsdlJson.definitions.types.schema) {
    ...
    if (mergeLen > 0) {
      ...
      if (!Array.isArray(wsdlJson.definitions.types.schema)) {
        const firstEntry = wsdlJson.definitions.types.schema;
...
function isWSAddressing(wsdlJson) {
  ...
  const binLen = defBindings.length;
