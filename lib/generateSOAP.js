...
function generateSOAP(serviceName, wsdlId, wsdlJson, wsdlDefNamespaces, globalNamespaces, serviceJSON, dict, refMap, options) {
  ...
  const swagger = initializeSwagger(serviceName, title, wsdlId, options.type);
  ...
  const bindings = {};
  const defBindings = u.makeSureItsAnArray(wsdlJson.definitions.binding);
  for (let j = 0; j < defBindings.length; j++) {
    const binding = defBindings[j];
    let bindingType = binding['undefined'].type;
    bindingType = u.stripNamespace(bindingType);
    const usingAddressing = !!binding.UsingAddressing;
    const operations = u.makeSureItsAnArray(binding.operation);
    const ops = [];
  ...
  const portTypes = {};
  const defPortTypes = u.makeSureItsAnArray(wsdlJson.definitions.portType);
  for (let j = 0; j < defPortTypes.length; j++) {
    const portType = defPortTypes[j];
    const operations = u.makeSureItsAnArray(portType.operation);
    const ops = [];
    for (let i = 0; i < operations.length; i++) {
      const operation = operations[i];
      ops.push({
  ...
  const messages = {};
  const defMessages = u.makeSureItsAnArray(wsdlJson.definitions.message);
  for (let j = 0; j < defMessages.length; j++) {
    const message = defMessages[j];
    const localNamespaces = getNamespaces(message, wsdlDefNamespaces);
    message.xmlns = localNamespaces;
    const msgNsName = dictionary.resolveNameInNamespace(message['undefined'].name, 'wsdl', localNamespaces, globalNamespaces, message.tns ? message.tns : tns);
    messages[msgNsName] = message;

    // Syntax check of the message
    const parts = u.makeSureItsAnArray(message.part);
  ...
  const defServices = u.makeSureItsAnArray(wsdlJson.definitions.service);
  for (let k = 0; k < defServices.length; k++) {
    const service = defServices[k];
    if (service['undefined'].name == serviceName) {
      ...
      for (let l = 0; l < considerPorts.length; l++) {
        const port = considerPorts[l];
        const bindingName = u.stripNamespace(port['undefined'].binding);
        if (bindings[bindingName] && bindings[bindingName].binding &&
          ...
          const b = bindings[bindingName].binding.binding['undefined'];
          const isSOAP = (b &&
                       (b.transport ||
                        b.style == 'document' ||
                        b.style == 'rpc'));
      ...
      const portName = generatePort(swagger, serviceName, ports, globalNamespaces, serviceJSON, dict);
      const portValue = '{' + tns + '}' + portName;
      const isSoap12 = isSoap12Port(portName, serviceName, globalNamespaces, serviceJSON);
      ...
      for (const bindingName in bindingLookup) {
        const pNames = bindingLookup[bindingName];
        let matchesPortName = false;
        for (let y = 0; y < pNames.length; y++) {
          const pName = pNames[y];
          if (pName == portName) {
        ...
        if (matchesPortName) {
          ...
          if (b) {
            ...
            if (b.binding.binding && b.binding.binding['undefined']) {
              const style = b.binding.binding['undefined'].style;
              if (style && style == 'rpc') {
      ...
      const portTypeOperations = portTypes[portTypeName].operations;
      const portType = portTypes[portTypeName].portType;
      const ptNamespaces = getNamespaces(portType, wsdlDefNamespaces);

      const usedMessages = {};
      const msgNameLookup = {};
      const msgNameOperLookup = {};
      ...
      for (let m = 0; m < portTypeOperations.length; m++) {
        const op = portTypeOperations[m];
        const bindingOper = getMatchingBindingOperation(op.operation['undefined'].name, bindingOperations);
        const boXMLNS = getNamespaces(bindingOper, wsdlDefNamespaces);
        const boTNS = bindingOper.tns || tns;
        if (bindingOper === null) {
        ...
        if (bindingOper.operation.operation && bindingOper.operation.operation['undefined']) {
          ...
          const bindingOpStyle = bindingOper.operation.operation['undefined'].style;
        ...
        const inputBody = getBody(bindingOper.operation.input);
        const outputBody = getBody(bindingOper.operation.output);
        const faultBody = getBody(bindingOper.operation.fault);

        // Get the part list for the input, output, fault of the binding operation
        const inputPartList = getParts(inputBody);
        const outputPartList = getParts(outputBody);
        const faultPartList = getParts(faultBody);
        ...
        const ptTNS = portType && portType.tns ? portType.tns : tns;
        if (op.operation.input) {
          ...
          if (isDocumentStyle) {
            ...
            const inputContentParts = getMimeContentParts(op.operation['undefined'].name, bindingOper.operation.input, dict);
            const opMessageInput = getMessagePartType(inputMessage, messages, inputPartList, globalNamespaces, inputContentParts, dict, tns, false);
        ...
        if (op.operation.output) {
          ...
          if (isDocumentStyle) {
            // message type comes from the message part type in document style
            const outputContentParts = getMimeContentParts(op.operation['undefined'].name, bindingOper.operation.output, dict);
            const opMessageOutput = getMessagePartType(outputMessage, messages, outputPartList, globalNamespaces, outputContentParts, dict, tns, false);
        ...
        if (op.operation.fault) {
          const faults = u.makeSureItsAnArray(op.operation.fault);
          const faultLen = faults.length;
          for (let n = 0; n < faultLen; n++) {
            const fault = faults[n];
            ...
            const faultMessageEntry = {
              message: faultMessage,
              name: faultMessageName,
            };
            if (isDocumentStyle) {
              const faultContentParts = getMimeContentParts(op.operation['undefined'].name, bindingOper.operation.fault, dict);
              const opMessageFault = getMessagePartType(faultMessage, messages, faultPartList, globalNamespaces, faultContentParts, dict, tns, true);
        ...
        const operName = inputMessage ? msgNameOperLookup[inputMessage] : op.name;
        const operNamespace = inputBody && !isDocumentStyle ? inputBody['undefined'].namespace : null;
        ...
        const opDescription = op.operation.documentation ? u.cleanupDocumentation(op.operation.documentation, req) : '';
        ...
        const inputDefinition = getEnvelopeBodyTemplate(isSoap12, {
        ...
        if (inputMessage) {
          const inMsgLen = inputMessageName.length;
          let inMsgPropName = '';
          let inMsgName = null;
          ...
          const inputHeaders = getHeaders(bindingOper.operation.input);
          if (inputHeaders) {
            const headers = u.makeSureItsAnArray(inputHeaders);
            for (let x = 0; x < headers.length; x++) {
              ...
              const headerMessageName = dictionary.resolveNameInNamespace(header['undefined'].message, 'wsdl', boXMLNS, globalNamespaces, boTNS);
              const headerPart = header['undefined'].part;
              wsiBindingHeaderCheck(header, op.operation['undefined'].name, dict, req);
              const headerMessage = messages[headerMessageName];
              if (headerMessage) {
                const headerParts = u.makeSureItsAnArray(headerMessage.part);
                for (let z = 0; z < headerParts.length; z++) {
                  const hdrPart = headerParts[z];
                  const hdrPartName = hdrPart['undefined'].name;
                  if (hdrPartName == headerPart) {
                    // found a part match, now add to header definition
                    const hdrElemName = u.stripNamespace(hdrPart['undefined'].element);
                    if (hdrElemName) {
                      const hdrElemNsName = dictionary.bestMatch(hdrPart['undefined'].element, 'element', headerMessage, dict, globalNamespaces);
        ...
        const outputDefinition = getEnvelopeBodyTemplate(isSoap12, {});
        ...
        if (outputMessage) {
          ...
          for (let ab = 0; ab < outputMessageName.length; ab++) {
            const outMsgName = outputMessageName[ab];
          ...
          if (!outputHeaders) {
            ...
            const headersOut = u.makeSureItsAnArray(outputHeaders);
            for (let x = 0; x < headersOut.length; x++) {
              const headerOut = headersOut[x];
              if (headerOut['undefined'] && headerOut['undefined'].use === 'encoded') {
                R.error(req,
                    g.http(u.r(req)).f('The \'use=encoded\' attribute setting is ignored on wsdl \'operation\' %s. This is a violation of a WS-I Rule (R2706 A wsdl:binding in a DESCRIPTION MUST use the value of "literal" for the use attribute in all wsoap11:body, wsoap11:fault, wsoap11:header and wsoap11:headerfault elements).', op.operation['undefined'].name));
              }
              const headerOutMessageName = dictionary.resolveNameInNamespace(headerOut['undefined'].message, 'wsdl', boXMLNS, globalNamespaces, boTNS);
              const headerOutPart = headerOut['undefined'].part;
              const headerOutMessage = messages[headerOutMessageName];
              if (headerOutMessage) {
                const headerOutParts = u.makeSureItsAnArray(headerOutMessage.part);
                for (let z = 0; z < headerOutParts.length; z++) {
                  const hdrOutPart = headerOutParts[z];
                  const hdrOutPartName = hdrOutPart['undefined'].name;
                  if (hdrOutPartName == headerOutPart) {
                    // found a part match, now add to header definition
                    const hdrOutElemName = u.stripNamespace(hdrOutPart['undefined'].element);
                    if (hdrOutElemName) {
                      const hdrOutElemNsName = dictionary.bestMatch(hdrOutPart['undefined'].element, 'element', headerOutMessage, dict, globalNamespaces);
        ...
        if (faultMessages.length > 0 ||
          ...
          const faultDefinition = getEnvelopeBodyFaultTemplate(isSoap12, {});
          ...
          const faultHeaderDefinition = {
            type: 'object',
            properties: {},
            description: 'Fault header for wsdl operation ' + operName,
          };
          const fHeaders = getHeaders(bindingOper.operation.fault);
          if (fHeaders) {
            swagger.definitions[headerFaultNSName] = faultHeaderDefinition;
            const headers = u.makeSureItsAnArray(fHeaders);
            for (let x = 0; x < headers.length; x++) {
              const header = headers[x];
              if (header['undefined'].use === 'encoded') {
                R.error(req,
          ...
          const faultHeaders = getFaultHeaders(bindingOper.operation.input, bindingOper.operation.output, bindingOper.operation.fault);
          if (!faultHeaders) {
            ...
            const headersFault = u.makeSureItsAnArray(faultHeaders);
            for (let x = 0; x < headersFault.length; x++) {
              const headerFault = headersFault[x];
              if (headerFault['undefined'] && headerFault['undefined'].use === 'encoded') {
                R.error(req,
                    g.http(u.r(req)).f('The \'use=encoded\' attribute setting is ignored on wsdl \'operation\' %s. This is a violation of a WS-I Rule (R2706 A wsdl:binding in a DESCRIPTION MUST use the value of "literal" for the use attribute in all wsoap11:body, wsoap11:fault, wsoap11:header and wsoap11:headerfault elements).', op.operation['undefined'].name));
              }
              const headerFaultMessageName = dictionary.resolveNameInNamespace(headerFault['undefined'].message, 'wsdl', boXMLNS, globalNamespaces, boTNS);
              const headerFaultPart = headerFault['undefined'].part;
              wsiBindingHeaderCheck(headerFault, op.operation['undefined'].name, dict, req);
              const headerFaultMessage = messages[headerFaultMessageName];
              if (headerFaultMessage) {
                const headerFaultParts = u.makeSureItsAnArray(headerFaultMessage.part);
                const hpartFaultLen = headerFaultParts.length;
                for (let z = 0; z < hpartFaultLen; z++) {
                  const hdrFaultPart = headerFaultParts[z];
                  const hdrFaultPartName = hdrFaultPart['undefined'].name;
                  if (hdrFaultPartName == headerFaultPart) {
                    // found a part match, now add to header definition
                    const hdrFaultElemName = u.stripNamespace(hdrFaultPart['undefined'].element);
                    if (hdrFaultElemName) {
                      const hdrFaultElemNsName = dictionary.bestMatch(hdrFaultPart['undefined'].element, 'element', headerFaultMessage, dict, globalNamespaces);
          ...
          for (let p = 0; p < faultMessages.length; p++) {
            const faultMsg = faultMessages[p];
      ...
      for (const messageName in usedMessages) {
        const msgNSName = msgNameLookup[messageName] || messageName;

        const prefix = u.getPrefixForNamespace(tns, globalNamespaces);
        const msgDefinition = {
          xml: {
            namespace: tns,
            prefix: prefix,
        ...
        const parts = u.makeSureItsAnArray(msg.part);
        for (let i = 0; i < parts.length; i++) {
          const part = parts[i];
          const partName = part['undefined'].name;
          ...
          const partNSName = dictionary.bestMatch(partType, kind, msg, dict, globalNamespaces);
          const dictEntry = dict.dictEntry[partNSName];
          ...
          if (partNameOK) {
            ...
            if (dictEntry) {
              ...
              const xso = genDefs.generateSwaggerXSO(dictEntry, dict, refMap, globalNamespaces, referencingContext);
              ...
              const xso = genDefs.mapXSDTypeToSwagger(u.stripNamespace(partType), dict);
              u.extendObject(property, xso);
            }
            if (isDocumentStyle) {
...
function wsdlConstructNotFound(name, wsdlConstruct, req, dict) {
  const msg = g.http(u.r(req)).f('Could not find wsdl %s with the name %s. This is a violation of a WS-I Rule (R2101 A DESCRIPTION MUST NOT use QName references to WSDL components in namespaces that have been neither imported, nor defined in the referring WSDL document).', wsdlConstruct, name);
  R.error(req, msg);
}
...
function wsiBindingHeaderCheck(h, operName, dict, req) {
  if (h['undefined'].part === null) {
    const msg = g.http(u.r(req)).f('A header or headerFault in binding operation %s is missing the part attribute. This is a violation of a WS-I Rule (R2720 A wsdl:binding in a DESCRIPTION MUST use the part attribute with a schema type of "NMTOKEN" on all contained wsoap11:header and wsoap11:headerfault elements).',
        operName);
    R.error(req, msg);
  }
  if (h['undefined'].parts) {
    const msg = g.http(u.r(req)).f('A header or headerFault in binding operation %s has a parts attribute. This is a violation of a WS-I Rule (R2749 A wsdl:binding in a DESCRIPTION MUST NOT use the parts attribute on contained wsoap11:header and wsoap11:headerfault elements).',
        operName);
    R.error(req, msg);
  }
}
...
function generatePort(swagger, serviceName, ports, namespaces, serviceJSON, dict) {
  ...
  let portName = '';
  if (ports.length > 0) {
    const usedPort = ports[0];
    portName = usedPort['undefined'].name;
    const isSoap12 = isSoap12Port(portName, serviceName, namespaces, serviceJSON);
    if (isSoap12) {
    ...
    if (usedPort.address) {
      const portLoc = usedPort.address['undefined'].location;
      if (portLoc) {
    ...
    if (ports.length > 1) {
      const msg = g.http(u.r(req)).f('The wsdl \'service\' has multiple \'ports\'. The api is generated using information in the first soap \'port\'.');
      R.info(req, msg);
    }
  }
  return portName;
...
function setTargetUrl(swagger, url) {
  const execute = swagger['x-ibm-configuration'].assembly.execute;
  if (execute[0].proxy) {
...
function isSoap12Port(portName, serviceName, namespaces, serviceJSON) {
  const prefixSOAP12 = getSoap12Prefix(namespaces);
  if (prefixSOAP12) {
    if (serviceJSON && serviceJSON.service) {
      ...
      for (let i = 0; i < serviceJSON.service.length; i++) {
        const service = serviceJSON.service[i];
              ...
              if (port && port['undefined'] && port['undefined'].name === portName) {
                if (port.address) {
                  ...
                  if (address && address['undefined'] && address['undefined'].__name__) {
                    const words = _.split(address['undefined'].__name__, ':');
...
function getMatchingBindingOperation(name, bindingOperations) {
  ...
  if (name && bindingOperations) {
    const len = bindingOperations.length;
    for (let i = 0; i < len; i++) {
      const op = bindingOperations[i];
...
function getBody(inoutfault) {
  if (!inoutfault) {
    ...
    for (let p = 0; p < inoutfault.multipartRelated.part.length; p++) {
      const part = inoutfault.multipartRelated.part[p];
      if (part.body) {
        return part.body;
      }
    }
...
function getHeaders(inout) {
  if (!inout) {
    ...
    for (let p = 0; p < inout.multipartRelated.part.length; p++) {
      const part = inout.multipartRelated.part[p];
      if (part.header) {
        return part.header;
      }
    }
...
function getFaultHeaders(input, output, fault) {
  const faultHeaders = [];
  let inHeaders = getHeaders(input);
  let outHeaders = getHeaders(output);
  let fhs = getHeaders(fault);
  let i; let j; let hfaults;
  if (inHeaders) {
...
function getMessagePartType(messageNsName, messages, partList, namespaces, contentPartList, dict, tns, forFault) {
  ...
  const ret = [];
  let opMsgParts;
  const opMessage = messages[messageNsName];
  if (opMessage) {
  ...
  if (opMessage) {
    const len = opMsgParts.length;
    for (let i = 0; i < len; i++) {
      const opMsgPart = opMsgParts[i];
      const opPartName = opMsgPart['undefined'].name;
      if (contentPartList.indexOf(opPartName) < 0) {
        ...
        if (partNameOK) {
          ...
          let opPartType = opMsgPart['undefined'].type;
          if (opPartType) {
            if (forFault) {
              R.info(req,
...
function getParts(body) {
  if (body) {
    const rawInputPartList = body['undefined'].parts;
    if (rawInputPartList) {
...
function getEnvelopeBodyFaultTemplate(soap12, header) {
  const template = getEnvelopeBodyTemplate(soap12, header);
  if (soap12) {
...
function patchPaths(swagger, dict) {
  ...
  for (let i = 0; i < dict.pathInfo.length; i++) {
    const pathItem = dict.pathInfo[i];
    const pathOpName = pathItem.operationName;
    let pathOpNS = pathItem.operationNS;
    if (!pathOpNS) {
      const pathInputs = pathItem.inputMessages;
      if (pathInputs && pathInputs.length > 0) {
        const nsName = pathInputs[0].type;
    ...
    const soapOperation = '{' + pathOpNS + '}' + pathOpName;
