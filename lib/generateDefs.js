...
const UNQUALNS = 'UNQUALIFIED_NAME_SPACE';

// Child descriptions place on parent or on Child
const CHILD_DESCRIPTION_PLACED_ON_PARENT = false;
...
function addReference(refMap, nsName, referencingContext) {
  ...
  if (!refMap[nsName]) {
    ...
    let found = false;
    const len = refMap[nsName].referencingContexts.length;
    for (let i = 0; i < len; i++) {
...
function generateSwaggerDefinitions(definitions, refMap, dict, namespaces, oldRefMap, totalRefMap) {
  ...
  for (let i = 0; i < len; i++) {
    ...
    // and which have not.
    const nameValue = refList[i];
    const nsName = nameValue.name;
    const value = nameValue.value;

    const rcLen = value.referencingContexts.length;
    for (let j = value.offset; j < rcLen; j++) {
...
function generateRootDefinition(nsName, referencingContext, definitions, refMap, dict, namespaces) {
  ...
  if (dictEntry) {
    ...
    if (!definitions[definitionName]) {
      const referencingContextForXML = u.deepClone(referencingContext);
  ...
  if (dictEntry.for === 'typedef') {
    ...
    const dDictEntry = dict.dictEntry[dictEntry.typeNSName];
    const inPolyTree = dDictEntry &&
    ...
    if (inPolyTree) {
      if (dDictEntry.ancType) {
        if (dict.createOptions.v3discriminator) {
          ...
          const ancDictEntry = dict.dictEntry[dDictEntry.ancType];
          const ancNSName = ancDictEntry.refNSName;
      ...
      if (dDictEntry.subTypes) {
        if (dict.createOptions.v3discriminator) {
          ...
          for (let i = 0; i < dDictEntry.subTypes.length; i++) {
            ...
            const descDictEntry = dict.dictEntry[dDictEntry.subTypes[i]];
            const descNSName = descDictEntry.refNSName;
...
function handleUnionRef(union, dictEntry, dict, refMap, xso, namespaces, tns) {
  ...
  if (union.simpleType) {
    ...
    for (let i = 0; i < union.simpleType.length; i++) {
      ...
      const simpleXSO = generateSwaggerXSO(simpleSchema, dict, refMap, namespaces, {});
      xso.anyOf.push(simpleXSO);
    }
  }
  if (union['undefined'] && union['undefined'].memberTypes) {
    const memberTypes = _.split(union['undefined'].memberTypes, ' ');
    for (let i = 0; i < memberTypes.length; i++) {
      const nsName = dictionary.bestMatch(memberTypes[i], 'typedef', dictEntry, dict, namespaces);
      const isXSD = dictionary.isXSDType(memberTypes[i], namespaces, dictEntry.xmlns);
      if (dict.dictEntry[nsName]) {
        const refNSName = getDefinitionName(nsName, {}, namespaces);
        addReference(refMap, nsName, {});
        xso.anyOf.push({
          $ref: '#/definitions/' + refNSName,
        });
      } else if (isXSD) {
        const xsdXSO = mapXSDTypeToSwagger(u.stripNamespace(memberTypes[i]), dict);
...
function generateSwaggerXSO_forComplexType(dictEntry, dict, refMap, namespaces, referencingContextForXML) {
  ...
  let mixedContentMappedToString = false;
  if (dictEntry.schemaType === 'complex' && dictEntry.schema['undefined'] && dictEntry.schema['undefined'].mixed === 'true') {
    if (dictEntry.schema.sequence ||
      ...
      if (dictEntry.schema.sequence) {
        const s = dictEntry.schema.sequence;
        if (s.any &&
...
function generateSwaggerXSO_forSubstitutionGroup(dictEntry, dict, refMap, namespaces, referencingContextForXML) {
  ...
  for (let i = 0; i <= dictEntry.nsNames.length; i++) {
    const elemNSName = dictEntry.nsNames[i];
    const elemDictEntry = dict.dictEntry[elemNSName];
    if (elemDictEntry && !elemDictEntry.tagInfo.abstract) {
      const propName = elemDictEntry.tagInfo.name;
  ...
  for (let i = 0; i <= dictEntry.nsNames.length; i++) {
    ...
    const elemDictEntry = dict.dictEntry[elemNSName];
    if (elemDictEntry && !elemDictEntry.tagInfo.abstract) {
      const elemXSO = newXSO(xso.oneOf, true);
      let propName = elemDictEntry.tagInfo.name;
      const referencingContext = {};
...
function detectMultipleSGAC(schema, dictEntry, req) {
  ...
  const constructs = ['sequence', 'all', 'choice', 'group'];
...
function handleSimpleContentRef(simpleContent, dictEntry, dict, refMap, xso, namespaces, tns) {
  ...
  const extension = dictEntry.schema.simpleContent.extension;
  const restriction = dictEntry.schema.simpleContent.restriction;
  if (extension) {
    delete xso.type;
    delete xso.properties;
    xso.allOf = [{
    ...
    const baseType = dictionary.resolveNameInNamespace(extension['undefined'].base, 'typedef', dictEntry.xmlns, namespaces, null, dict);
    if (baseType) {
      if (dict.dictEntry[baseType]) {
        const baseNSName = baseType;
        addReference(refMap, baseType, {});
        newXSO(xso.allOf);
        xso.allOf[0] = {$ref: '#/definitions/' + baseNSName};
      } else {
        const simpleType = mapXSDTypeToSwagger(u.stripNamespace(extension['undefined'].base), dict);
...
function handleComplexContentRef(complexContent, dictEntry, dict, refMap, xso, namespaces, tns) {
  ...
  xso.type = 'object';
  xso.properties = {};
  const restriction = complexContent.restriction;
  ...
  if (restriction) {
    ...
    if (baseType) {
      ...
      if (baseType == 'array') {
        ...
        const attribute = restriction.attribute;
        if (attribute) {
          for (const attr in attribute['undefined']) {
            const attrName = u.stripNamespace(attr).toLowerCase();
            if (attrName == 'arraytype') {
              let rawArrayType = attribute['undefined'][attr];
              if (rawArrayType.indexOf('[]', rawArrayType.length - 2) != -1) {
                rawArrayType = rawArrayType.substr(0, rawArrayType.length - 2);
              }
              const arrayType = dictionary.bestMatch(rawArrayType, 'type', dictEntry.schema, dict, namespaces);
              xso.type = 'array';
              delete xso.properties;
              xso.items = {};
              if (dict.dictEntry[arrayType]) {
                ...
                const swaggerType = mapXSDTypeToSwagger(u.stripNamespace(rawArrayType), dict);
        ...
        const baseType = dictionary.bestMatch(restriction['undefined'].base, 'typedef', dictEntry, dict, namespaces);
        if (!baseType || !dict.dictEntry[baseType] || !dict.dictEntry[baseType].schema) {
          ...
          const baseSchema = dict.dictEntry[baseType];
          const baseNSName = baseType;
          addReference(refMap, baseType, {});
  ...
  if (extension) {
    ...
    const baseType = dictionary.bestMatch(extension['undefined'].base, 'typedef', dictEntry, dict, namespaces);
    if (baseType) {
      if (dict.dictEntry[baseType]) {
...
function processSchemaSimpleRestriction(restriction, inXSO, dict, dictEntry, refMap, namespaces) {
  ...
  if (!isXSD && typeDictEntry) {
    if (dictEntry.schema['undefined'] && dictEntry.schema['undefined'].name == xso.type) {
      ...
      const swaggerType = mapXSDTypeToSwagger(xso.type, dict);
      ...
      if (isPrimitiveType(typeDictEntry, namespaces)) {
        ...
        const primitiveRef = generateSwaggerXSO(typeDictEntry, dict, refMap, namespaces);
        delete xso.type;
        ...
        const baseXSO = generateSwaggerXSO(typeDictEntry, dict, refMap, namespaces);
        delete xso.type;
        ...
        if (xso.allOf) {
          ...
          if (facetxso.type == 'object' && xso.allOf[0]['$ref']) {
            const nsName = u.getDefNameFromRef(xso.allOf[0]['$ref']);
            if (dict.dictEntry[nsName]) {
        ...
        const ref = '#/definitions/' + xso.type;
        delete xso.type;
        xso.allOf = [];
        xso.allOf[0] = {$ref: ref};
    ...
    const swaggerType = mapXSDTypeToSwagger(u.stripNamespace(restriction['undefined'].base), dict);
    u.extendObject(xso, swaggerType);
  }

  // Now process the facets
  if (restriction.pattern) {
    const patterns = u.makeSureItsAnArray(restriction.pattern);
    const patLen = patterns.length;
    if (patLen == 1) {
...
function processOccurrence(schema) {
  const occur = {minOccurs: 1, maxOccurs: 1};
...
function handleSequenceRef(sequence, dictEntry, dict, refMap, xsoList, namespaces, tns) {
  const occur = processOccurrence(sequence);
  const currXSOList = getXSOList(occur, xsoList, 'sequence');
...
function handleSequenceContent(sequence, dictEntry, dict, refMap, xsoList, namespaces, tns) {
  ...
  if (sequence.any) {
    kinds++;
    const anys = u.deepClone(u.makeSureItsAnArray(sequence.any));
  ...
  if (sequence.element) {
    kinds++;
    const elements = u.deepClone(u.makeSureItsAnArray(sequence.element));
  ...
  if (sequence.choice) {
    const choices = u.deepClone(u.makeSureItsAnArray(sequence.choice));
  ...
  if (sequence.group) {
    const groups = u.deepClone(u.makeSureItsAnArray(sequence.group));
  ...
  if (sequence.sequence) {
    const sequences = u.deepClone(u.makeSureItsAnArray(sequence.sequence));
...
function anyElementsOrAttributes(isAny, any, dict, tns) {
  ...
  if (any['undefined'] && (!any['undefined'].processContents || any['undefined'].processContents === 'strict')) {
    ...
    for (let i = 0; i < namespaces.length; i++) {
      const namespace = namespaces[i].trim();
      if (namespace == '##any') {
        return [];
  ...
  for (let i = 0; i < ns.length; i++) {
    const list = (isAny) ? dict.schemaElements[ns[i]] : dict.schemaAttributes[ns[i]];
    rc = _.union(rc, list);
  }
  if (rc.length > 50) {
    return [];
...
function isElementSubstitutionGroup(schema, dictEntry, dict, namespaces) {
  if (schema['undefined'] && schema['undefined'].ref) {
    const nsName = dictionary.bestMatch(schema['undefined'].ref, 'element', dictEntry, dict, namespaces);
    const eDictEntry = dict.dictEntry[nsName];
...
function handleChoiceRef(choice, dictEntry, dict, refMap, xsoList, namespaces, tns) {
  const occur = processOccurrence(choice);
  const currXSOList = getXSOList(occur, xsoList, 'choice');
...
function handleChoiceContent(choice, dictEntry, dict, refMap, xsoList, namespaces, tns) {
  ...
  if (choice.any) {
    basicPattern = false;
    const anys = u.deepClone(u.makeSureItsAnArray(choice.any));
  ...
  if (choice.element) {
    const elements = u.deepClone(u.makeSureItsAnArray(choice.element));
  ...
  if (choice.sequence) {
    basicPattern = false;
    const sequence = u.deepClone(u.makeSureItsAnArray(choice.sequence));
  ...
  if (choice.group) {
    basicPattern = false;
    const group = u.deepClone(u.makeSureItsAnArray(choice.group));
  ...
  if (basicPattern && oneOf && oneOf.length > 1 && !dict.createOptions.v3oneOf) {
    ...
    oneOf.forEach(function(xso) {
      if (xso.properties) {
        const propNames = Object.keys(xso.properties);
...
function handleStrictAnyRef(any, dictEntry, dict, refMap, xsoList, namespaces, tns) {
  const occur = processOccurrence(any);
  const currXSOList = getXSOList(occur, xsoList, 'any');
...
function handleStrictAnyContent(any, dictEntry, dict, refMap, xsoList, namespaces, tns) {
  ...
  const nsNames = anyElementsOrAttributes(true, any, dict, tns);
  let first = true;
  ...
  for (let i = 0; i < nsNames.length; i++) {
    ...
    const propName = nsNames[i].substring(0, nsNames[i].indexOf('_element'));
    const o = xso.allOf[1].oneOf[xso.allOf[1].oneOf.length - 1];
...
function handleAllRef(all, dictEntry, dict, refMap, xsoList, namespaces, tns) {
  const occur = processOccurrence(all);
  const currXSOList = getXSOList(occur, xsoList, 'all');
...
function handleAllContent(all, dictEntry, dict, refMap, xsoList, namespaces, tns) {
  ...
  if (all['undefined']) {
    ...
    const minOccurs = all['undefined'].minOccurs;
    if (minOccurs === 0) {
      required = false;
    }
  }
...
function handleSubstitutionGroupRef(schema, dictEntry, dict, refMap, xsoList, namespaces, tns) {
  ...
  const sgNSName = dictionary.resolveNameInNamespace(schema['undefined'].ref, 'substitutionGroup', dictEntry.xmlns, namespaces, dictEntry.tns, dict);
  const sgDictEntry = dict.dictEntry[sgNSName];
  if (!sgDictEntry) {
    return;
  }
  const occur = processOccurrence(schema);

  const xso = newXSO(xsoList, true);
  delete xso.type;
  delete xso.properties;
  delete xso.xml;
...
function handleGroupRef(group, dictEntry, dict, refMap, xsoList, namespaces, tns) {
  ...
  const groupNSName = dictionary.resolveNameInNamespace(group['undefined'].ref, 'group', dictEntry.xmlns, namespaces, dictEntry.tns, dict);
  const groupDictEntry = dict.dictEntry[groupNSName];
  if (!groupDictEntry) {
  ...
  const occur = processOccurrence(group);
  const xso = newXSO(xsoList, true);
  delete xso.type;
  delete xso.properties;
  delete xso.xml;
...
function handleGroupContent(group, dictEntry, dict, refMap, xsoList, namespaces, tns) {
  ...
  // look for nested all
  if (group.all) {
    const alls = u.makeSureItsAnArray(group.all);
    const allLen = alls.length;
...
function handleAttributeGroupRef(inGroup, dictEntry, dict, refMap, xsoList, namespaces, tns) {
  const attrGroups = u.makeSureItsAnArray(inGroup);
  for (let i = 0; i < attrGroups.length; i++) {
    const attrGroup = attrGroups[i];
    const attrRef = attrGroup['undefined'].ref;
    if (attrRef) {
      const nsName = dictionary.bestMatch(attrRef, 'attributeGroup', dictEntry, dict, namespaces);
      const xso = newXSO(xsoList); // Create a new xso for the substitution group
      delete xso.type;
      delete xso.properties;
      delete xso.xml;
...
function handleAttributeRef(inAttr, dictEntry, dict, refMap, xsoList, namespaces, tns) {
  const attributes = u.makeSureItsAnArray(inAttr);
  const len = attributes.length;
  let xso = newXSO(xsoList);
  xso.xml = {namespace: ''};
  for (let i = 0; i < len; i++) {
    const attribute = attributes[i];
    let qualifiedAttr = dictEntry.qualifiedAttr;
    ...
    let name = attribute['undefined'].name;
    ...
    const attrRef = attribute['undefined'].ref;
    let nsName = null;
    let isXSD = false;
    ...
    if (attrRef) {
      ...
      // 2. Attribute is a built-in type
      const swaggerType = mapXSDTypeToSwagger(u.stripNamespace(attribute['undefined'].type), dict);
      ...
      if (attribute.simpleType) {
        ...
        const inlineRet = generateSwaggerXSO(simpleSchema, dict, refMap, namespaces);
    ...
    if (attribute.annotation) {
      ...
      if (attribute.annotation.documentation) {
        const doc = u.cleanupDocumentation(attribute.annotation.documentation, dict.req);
...
function handleElement(element, dictEntry, dict, refMap, xsoList, namespaces, tns) {
  ...
  let childQualified = schemaQualified;
  const elemQualified = element['undefined'].form;
  if (elemQualified) {
  ...
  let elemNillable = false;
  const elemNillableAttr = element['undefined'].nillable;
  ...
  if (elemName) {
    ...
    if (element['undefined'].ref) {
      ...
      const tagInfo = dict.dictEntry[elemTypeOrRef] ? dict.dictEntry[elemTypeOrRef].tagInfo : null;
  ...
  if (!isXSD && elemTypeOrRef && dict.dictEntry[elemTypeOrRef]) {
    ...
    if (!isElemRef &&
      ...
      if (isPrimitiveType(refType, namespaces)) {
        const primitiveRef = generateSwaggerXSO(refType, dict, refMap, namespaces);
        ...
        const referencingNS = (childQualified) ? tns : UNQUALNS;
        ...
        detectPropertyCollision(elemName, xso.properties, element, dict.req);
        const dupType = getDefinitionName(elemTypeOrRef, {ns: referencingNS}, namespaces);
    ...
    const swaggerType = mapXSDTypeToSwagger(u.stripNamespace(element['undefined'].type), dict);
    detectPropertyCollision(elemName, xso.properties, element, dict.req);
  ...
  if ('complexType' in element) {
    ...
    const typeRef = generateSwaggerXSO(complexSchema, dict, refMap, namespaces, referencingContext);
  ...
  if (element.simpleType) {
    ...
    const referencingContextSimple = {
      ns: UNQUALNS,
    };
    if (childQualified) {
      referencingContextSimple.ns = tns;
    }
    const simpleTypeRef = generateSwaggerXSO(simpleSchema, dict, refMap, namespaces, referencingContextSimple);
  ...
  const item = (xso.properties[elemName].items != null) ? xso.properties[elemName].items : xso.properties[elemName];
  ...
  if (element.annotation) {
    ...
    if (element.annotation.documentation) {
      const doc = u.cleanupDocumentation(element.annotation.documentation, req);
  ...
  if (isAbstractElement) {
    ...
    if (xso.required && xso.required.length > 0) {
      const i = xso.required.indexOf(elemName);
      if (i >= 0) {
...
function processElementOccurrence(element, propXSO, elemNillable, dict) {
  ...
  const minElement = getOccurrence(element['undefined'].minOccurs);
  const maxElement = getOccurrence(element['undefined'].maxOccurs);

  // If the property is required add it to the required list
  if (minElement != 0) {
    for (const propName in propXSO) {
      const xso = propXSO[propName];
  ...
  const maxItems = maxElement;
  const minItems = minElement;

  if (maxItems == -1 || maxItems > 1) {
    // we have an array type - switch out the content
    for (const propName in propXSO) {
      const xso = propXSO[propName];
      const existingContent = xso.properties[propName];
...
function mapXSDTypeToSwagger(xsdType, dict) {
  dictionary.complexityLimitCheck(dict);
  let xso = dictionary.getXSDMapping(xsdType);
  if (!xso) {
    if (xsdType) {
...
function isPrimitiveType(dictEntry, namespaces) {
  ...
  if (dictEntry && dictEntry.schemaType && dictEntry.schema) {
    if (dictEntry.schemaType == 'simple' && dictEntry.schema.restriction) {
      const restriction = dictEntry.schema.restriction;
      if (restriction['undefined']) {
