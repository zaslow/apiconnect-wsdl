...
const NEVER = 'NEVER';
const IFNOTDEFAULT = 'IF-NOT-DEFAULT';
const IFENABLED = 'IF-ENABLED';
const ALWAYS = 'ALWAYS';
...
function create(_options, _defaults) {
  _options = _options || {};
  _defaults = _defaults || {};
  const options = _.merge({}, DEFAULTS, _defaults, _options);
...
function persistOptions(options, swagger) {
  ...
  const o = swagger['x-ibm-configuration']['x-ibm-apiconnect-wsdl'].options;
  for (const key in options) {
    if (OPTIONS[key]) {
