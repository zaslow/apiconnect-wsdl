...
function generateExamples(swagger, req) {
  ...
  for (const nsName in swagger.definitions) {
    ...
    if (xso.example) {
      try {
        const xmlSample = exampleXMLForType(nsName, swagger.definitions, detailMap, req);
        if (xmlSample) {
...
function generateElement(name, xso, definitions, nsStack, context, inOneOf, req) {
  ...
  if (xso.type === 'array') {
    ...
    push(nsStack, xso.xml);
    const ret = generateElement(name, xso, definitions, nsStack, context, inOneOf, req);
    pop(nsStack);
    ...
    const padding = padString(context.nesting);
    context.nesting++;
    push(nsStack, xso.xml);
    const xml = getXML(nsStack);
    name = xml.name || name;
    let tagName = name;
    ...
    const attrs = getAttributes(xso, definitions, {}, req);
    const primitiveContent = getPrimitiveContent(xso, definitions, req);
    const content = primitiveContent || getContent(xso, definitions, nsStack, context, false, req) || '';
    ...
    if (context.detailMap) {
      const t = u.getXSIType(xso, definitions);
    ...
    const startTag = (context.nesting === 0) ?
    ...
    // of the parser.
    const special = (content.indexOf('<!-- schema nesting too deep, truncated example -->') >= 0 &&
            content.indexOf('\n') < 0);
...
function getContent(xso, definitions, nsStack, context, inOneOf, req) {
  const primitive = getPrimitiveContent(xso, definitions, req);
  if (primitive) {
    return primitive;
  } else if (xso.allOf) {
...
function getOccurrenceComment(xso, inOneOf) {
  ...
  const maxItems = xso.maxItems ? xso.maxItems : 'unlimited';
  if (inOneOf) {
    minItems = 'zero';
  }
...
function getOneOfContent(xso, definitions, nsStack, context, inOneOf, req) {
  ...
  if (xso.oneOf) {
    ...
    const length = xso.discriminator ? 1 : xso.oneOf.length;
    if (length > 1) {
...
function getObjectContent(xso, definitions, nsStack, context, inOneOf, req) {
  let ret = '';
  let breadth = 0;
  const breadthLimit = context.breadthLimit;
  let choiceComment;
  if (xso.properties) {
    for (const propName in xso.properties) {
      const propXSO = xso.properties[propName];
...
function getAttributes(xso, definitions, prefixes, req) {
  ...
  if (xso) {
    ...
    if (xso.allOf || (xso.oneOf && xso.discriminator)) {
      const list = xso.allOf || xso.oneOf;
  ...
  if (properties) {
    for (const propName in properties) {
      ...
      if (prop.xml && prop.xml.attribute) {
        const value = getPrimitiveContent(prop, definitions, req);
        const shortName = prop.xml.name ? prop.xml.name : propName;
        let name = shortName;
...
function getPrimitiveContent(xso, definitions, req) {
  ...
  if (xso.allOf || xso.oneOf || xso.anyOf) {
    const list = xso.allOf || xso.oneOf || xso.anyOf;
    const length = xso.discriminator ? 1 : list.length;
    for (let i = 0; i < length; i++) {
      const prim = getPrimitiveContent(list[i], definitions, req);
      if (prim) {
        return prim;
      }
    }
...
function getRefXSO(ref, definitions, req) {
  ...
  const nsName = ref.substr(lastSlash + 1);
  const firstUnderscore = nsName.indexOf('_');
  const lastUnderscore = nsName.lastIndexOf('_');
  const prefix = nsName.substr(lastUnderscore + 1);
  const name = firstUnderscore > 0 ? nsName.substr(0, firstUnderscore) : nsName;
  const xso = definitions[nsName];
  if (!xso) {
